...
/* global force, locale_msg, nanobar, Nanobar, Headroom, $ */
/**
 * Raun-streamlined.js
 *
 * @author Kenrick
...
function getDbName (config) {
  if (config.language === 'www') {
    return config.project + 'wiki'
  }
  return config.language + 'wiki'
}
...
Model.prototype.init = function (view) {
  ...
  function setConfig (param) {
    if (parseInt(force[param], 10) === 0) {
      ...
      siteConfig[param] = that.getStorage(param)
      ...
      siteConfig[param] = that.config[param]
    }
  }
  setConfig('language')
  setConfig('project')
  ...
  var tempString = localStorage.getItem('config')
  var keys = null
  var disp = []

  if (tempString && force.locale && force.language && force.project) {
    this.config = JSON.parse(tempString)
    that.config.run = true
    this.config.language = siteConfig.language
    this.config.locale = siteConfig.locale
    this.config.project = siteConfig.project
  } else {
    this.config.language = siteConfig.language
    this.config.locale = siteConfig.locale
    this.config.project = siteConfig.project
  ...
  $('#show_more_entries').prop(
    'checked',
    this.config.tool.more_entries === true
  )
  ...
  this.tryORES(getDbName(this.config))
}
...
Model.prototype.getRCPolling = function (view) {
  var that = this
  view.displayBar(50)
  this.getDataPolling(view, 'rc', { from: 0, gtz: 0, lastRcId: 0 }, function (
    ret,
    data,
    callback
  ) {
    setTimeout(
      function process (ret, data, callback) {
        ...
        data.params.lastRcId = ret.lastRcId
        that.getDataPolling(view, 'rc', data.params, callback)
      }.bind(that, ret, data, callback),
      5000
    )
  })
}
...
Model.prototype.getUserPolling = function (view, group, callback) {
  ...
  var i
  this.getDataPolling(view, 'user', { group: group }, function (
    ret,
    data,
    callback
  ) {
...
Model.prototype.getStatPolling = function (view) {
  var that = this
  this.getDataPolling(view, 'stat', { method: 'stat' }, function (
    ret,
    data,
    callback
  ) {
...
Model.prototype.getORESOnce = function (view, revid) {
  ...
  $.ajax({
    type: 'GET',
    url: '//ores.wikimedia.org/scores/' + wiki + '/reverted/' + revid + '/',
    dataType: 'json',
    crossDomain: true,
    success: function (data) {
...
/**
 * Model: Wikimedia EventStream
 */
Model.prototype.initStream = function () {
  return new EventSource('https://stream.wikimedia.org/v2/stream/recentchange')
}
Model.prototype.getRCStream = function (view) {
  var stream = this.initStream()
  var that = this
  stream.onmessage = function (e) {
    var data = JSON.parse(e.data)
    if (data.wiki !== getDbName(that.config)) {
      return
    }
    data.config = that.config
    data.site = that.data
...
Model.prototype.updateFilter = function (view) {
  ...
  var len = this.data.filter.length
  var j
  for (j = 0; j < len; j++) {
    newConfig.show[this.data.filter[j]] = this.getFilter(this.data.filter[j])
  }
  for (j = 0; j < len; j++) {
    if (
      newConfig.show[this.data.filter[j]] &&
      !this.config.show[this.data.filter[j]]
    ) {
      view.showRC('.' + this.data['filter-class'][j])
    }
  }
  for (j = 0; j < len; j++) {
    if (!newConfig.show[this.data.filter[j]]) {
      view.hideRC('.' + this.data['filter-class'][j])
    }
  }
  // Update the data
  for (j = 0; j < len; j++) {
    this.config.show[this.data.filter[j]] = newConfig.show[this.data.filter[j]]
  }
  // Store the data
  localStorage.setItem('config', JSON.stringify(this.config))
}
...
function View () {
  ...
  // Show landing modal on unforced config
  if (
    parseInt(force.language, 10) === 0 ||
    parseInt(force.project, 10) === 0 ||
    parseInt(force.locale, 10) === 0
  ) {
    $('#landing').modal('show')
  }
...
View.prototype.displaySingleRC = function (data) {
  ...
  if (data.parsedcomment) {
    comment = data.parsedcomment.replace(
      /"\/wiki\//g,
      '"' + data.server_url + 'wiki/'
    )
  }

  data.pageid = getHashCode(data.server_url + data.title) // "stream" data does not have pageid, generate one from title
  ...
  if (data.hasOwnProperty('new') || data.type === 'new') {
    attr += 'new-art '
  }
  if (data.userhidden === undefined) {
    if (
      data.site.user.editor !== undefined &&
      data.site.user.editor.hasOwnProperty(data.user.toLowerCase())
    ) {
      attr += 'editor '
    }
    if (
      data.site.user.sysop !== undefined &&
      data.site.user.sysop.hasOwnProperty(data.user.toLowerCase())
    ) {
      attr += 'admin '
    }
  }
  ...
  if (data.userhidden === undefined) {
    ...
    userElem.setAttribute(
      'href',
      data.server_url + 'wiki/Special:Contributions/' + data.user
    )
  ...
  if (data.hasOwnProperty('minor') && !!data.minor) {
    cell[4].appendChild(
      this.createLabel(
        'label-primary',
        locale_msg('settings_minor_edits'),
        locale_msg('minor')
      )
    )
    cell[4].insertAdjacentHTML('beforeend', ' ')
  ...
  if (data.hasOwnProperty('anon')) {
    cell[4].appendChild(
      this.createLabel(
        'label-danger',
        locale_msg('settings_anon_edits'),
        locale_msg('anon')
      )
    )
    cell[4].insertAdjacentHTML('beforeend', ' ')
  ...
  if (data.userhidden === undefined) {
    if (
      ...
      cell[4].appendChild(
        this.createLabel(
          'label-default',
          locale_msg('settings_editor_edits'),
          locale_msg('editor')
        )
      )
      cell[4].insertAdjacentHTML('beforeend', ' ')
    ...
    if (
      ...
      cell[4].appendChild(
        this.createLabel(
          'label-info',
          locale_msg('settings_admin_edits'),
          locale_msg('admin')
        )
      )
      cell[4].insertAdjacentHTML('beforeend', ' ')
  ...
  if (data.config.language === '*') {
    cell[4].appendChild(
      this.createLabel(
        'label-default',
        data.server_url.slice(2, data.server_url.length - 5),
        data.server_url.slice(2, data.server_url.length - 5)
      )
    )
    cell[4].insertAdjacentHTML('beforeend', ' ')
  ...
  if (data.hasOwnProperty('tags') && data.tags.length > 0) {
    cell[4].insertAdjacentHTML('beforeend', ' (Tag: <i>' + data.tags + '</i>)')
  }
  ...
  if ($('.revid-' + data.old_revid).length > 0) {
    $(card).data(
      'oldest_revid',
      $('.pageid-' + data.pageid).last().data('oldest_revid')
    )
    ...
    if (parseInt($(card).data('oldest_revid'), 10) === 0) {
      cell[1].childNodes[0].setAttribute(
        'href',
        data.server_url + 'wiki/' + data.title
      )
    } else {
      cell[1].childNodes[0].setAttribute(
        'href',
        cell[1].childNodes[0]
          .getAttribute('href')
          .replace(/oldid=[0-9]*/, 'oldid=' + $(card).data('oldest_revid'))
      )
    }
    var combinedDiff = diff + this.calculateDiff('.pageid-' + data.pageid)
    diffClass = 'badge'
    if (combinedDiff > 0) {
      diffClass += ' size-pos'
    ...
    cell[0].textContent = (combinedDiff > 0 ? '+' : '') + combinedDiff
  ...
  for (j = 0; j < data.site.filter.length; j++) {
    if (
      attr.indexOf(data.site['filter-class'][j]) >= 0 &&
      !data.config.show[data.site.filter[j]]
    ) {
      showArticle = false
    }
    if (!showArticle) {
...
View.prototype.displayRC = function (data) {
  ...
  var baseSite =
    '//' + data.config.language + '.' + data.config.project + '.org/'
  var gtz = data.params.gtz
  var lastRcId = data.params.lastRcId
  var len = data.length
  var tz = gtz
  ...
  for (i = len - 1; i >= 0; i--) {
    if (lastRcId >= data[i].rcid) {
      continue
    }
    lastRcId = data[i].rcid
    tz = data[i].timestamp
    gtz = tz
    data[i].server_url = baseSite
  ...
  return { gtz: gtz, lastRcId: lastRcId }
}
  ...
  if (!data.error) {
    ...
    if ($('.revid-' + revid).hasClass('combined')) {
      ...
      setTimeout(function () {
        ...
        var pageidClass = $('.revid-' + revid + '.combined')
          .attr('class')
          .split(' ')
        ...
        var average = 0
        var current = null
        $('.' + pageidClass + '.combined-child').each(function () {
          current = parseFloat($(this).data('ores-score'), 10)
          if (!isNaN(current)) {
            // skip counting if not a number
            total += current
            number += 1
            average = total / number
          }
          // console.log(revid, pageidClass, total, number, average);
        })
        ...
        oresScoreCombined.setAttribute(
          'class',
          'list-group-item-text ores-score'
        )
        ...
        $(oresScoreCombined).insertAfter(
          '.revid-' + revid + '.combined > .user'
        )
      }, 1000)
    }
  }
...
View.prototype.displayStat = function (data) {
  ...
  var depth =
    data.edits *
    (data.pages - data.articles) *
    (data.pages - data.articles) /
    (data.articles * data.articles * data.pages)

  // construct message
  var msg = document.createElement('dl')
